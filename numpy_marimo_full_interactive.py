...
def _(mo):
    ...
    Kui kõik harjutused on proovitud, vajuta nuppu all.
    Vastused saadetakse automaatselt õpetajale ja salvestatakse Google Sheet'i.
    """)
    return
...
@app.cell
def _(mo):
    submit_url = "PASTE_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE"
    student_name = mo.ui.text(value="", label="Tudengi nimi (soovi korral)")
    student_email = mo.ui.text(value="", label="Tudengi e-post (soovi korral)")
    mo.vstack([student_name, student_email])
    return student_email, student_name, submit_url
...
def _(
    ...
    answers = [
        {"id": 2, "label": "Harjutus 2: 10 nulliga massiiv", "answer": q_zeros.value},
        {"id": 3, "label": "Harjutus 3: 10 ühte", "answer": q_ones.value},
        {"id": 4, "label": "Harjutus 4: 10 viiega massiiv", "answer": q_fives.value},
        {"id": 5, "label": "Harjutus 5: 5 kuni 10", "answer": q_arange.value},
        {"id": 6, "label": "Harjutus 6: paarisarvud 2..8", "answer": q_arange_even.value},
        {"id": 7, "label": "Harjutus 7: 4×4 (0..15)", "answer": q_reshape.value},
        {"id": 8, "label": "Harjutus 8: 5×5 identiteedimaatriks", "answer": q_eye.value},
        {"id": 9, "label": "Harjutus 9: 2 juhuslikku [0,1)", "answer": q_rand.value},
        {"id": 10, "label": "Harjutus 10: 30 normaaljaotuse arvu", "answer": q_randn.value},
        {"id": 11, "label": "Harjutus 11: 5×5 (1..25)/25", "answer": q_matdiv.value},
        {"id": 12, "label": "Harjutus 12: 10 punkti 0..1", "answer": q_linspace.value},
        {"id": 13, "label": "Harjutus 13: 6×6 (10..45)", "answer": q_matcreate.value},
        {"id": 14, "label": "Harjutus 14: slicing", "answer": q_sub.value},
        {"id": 15, "label": "Harjutus 15: üksik element", "answer": q_single.value},
        {"id": 16, "label": "Harjutus 16: veerutulp 4×1", "answer": q_col.value},
        {"id": 17, "label": "Harjutus 17: eelviimane rida", "answer": q_lastrow.value},
        {"id": 18, "label": "Harjutus 18: esimesed kaks rida", "answer": q_first2.value},
        {"id": 19, "label": "Harjutus 19: mat.sum()", "answer": q_matsum.value},
        {"id": 20, "label": "Harjutus 20: mat.std()", "answer": q_matstd.value},
        {"id": 21, "label": "Harjutus 21: mat.sum(axis=0)", "answer": q_sumaxis.value},
    ]

    def _attempted(s):
        return bool(s and str(s).strip())

    attempted_flags = [_attempted(a["answer"]) for a in answers]
    missing_ids = [str(a["id"]) for a, ok in zip(answers, attempted_flags) if not ok]
    total = len(answers)
    done = total - len(missing_ids)

    payload = {
        "submission_id": str(uuid.uuid4()),
        "submitted_at": datetime.datetime.now().isoformat(),
        "student_name": student_name.value.strip(),
        "student_email": student_email.value.strip(),
        "answers": answers,
    }
    payload_json = json.dumps(payload, ensure_ascii=False)

    has_submit_url = bool(submit_url) and not submit_url.startswith("PASTE_")
    all_attempted = done == total
    auto_ready = all_attempted and has_submit_url

    if not hasattr(js.window, "__SUBMITTED__"):
        js.window.__SUBMITTED__ = False
    js.window.__SUBMIT_URL__ = submit_url
    js.window.__SUBMIT_PAYLOAD__ = payload_json
    js.window.__SUBMIT_READY__ = auto_ready
    js.eval(
        """
        if (!window.__SUBMIT_HOOK__) {
          window.__SUBMIT_HOOK__ = true;
          window.addEventListener("pagehide", function () {
            try {
              if (window.__SUBMIT_READY__ && !window.__SUBMITTED__) {
                const blob = new Blob([window.__SUBMIT_PAYLOAD__], { type: "application/json" });
                const ok = navigator.sendBeacon(window.__SUBMIT_URL__, blob);
                if (ok) {
    ...
    status = f"**Täidetud:** {done}/{total}"
    if missing_ids:
        status += f" — puudu: {', '.join(missing_ids)}"

    confirm_btn = mo.ui.button(label="Kinnita, et oled kõik on valmis")
    mo.vstack([mo.md(status), confirm_btn])

    mo.stop(confirm_btn.value == 0, mo.md("*Vajuta nuppu, kui oled kõik proovinud.*"))

    if not has_submit_url:
        mo.md("**Viga:** `submit_url` ei ole seadistatud.")
        return
    if not all_attempted:
        mo.md("**Viga:** Kõik harjutused pole proovitud. Palun täida kõik väljad.")
        return

    if js.window.__SUBMITTED__:
        mo.md("**Info:** Vastused on juba saadetud.")
        return

    blob = js.Blob.new([payload_json], {"type": "application/json"})
    ok = js.navigator.sendBeacon(submit_url, blob)
    if ok:
        js.window.__SUBMITTED__ = True
        mo.md("**Aitäh!** Vastused saadetud.")
    else:
        mo.md("**Viga:** Saatmine ebaõnnestus. Palun proovi uuesti.")
    return
